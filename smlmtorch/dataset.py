...
class Dataset:
    ...
    def crlb_filter(self, max_crlb_xy, max_crlb_z=None, min_photons=None):
    ...
    def frc(self, zoom = 20, split_window=30, mask=None, **kwargs):
        ...
        """
        from smlmtorch.frc import FRC
        
        if mask is None and split_window is not None:
            mask = (self.frame // split_window) % 2 == 0
        
        return FRC(self.pos[:,:2], self.photons, zoom=zoom, mask=mask,
    ...
    def render(self, zoom=1, sigma=1, clipmax=1, clip_percentile=None):
    ...
    def frameConnect(self, distance_px, ndims=2):
        ...
        for i in tqdm.trange(nframes-1):
