import smlmtorch.detection as detection
import os
import smlmtorch.caching as cache
import torch
from smlmtorch.multipart_tiff import tiff_read_file, tiff_get_movie_size, tiff_get_image_size, tiff_get_filenames
import numpy as np
import tqdm
...
from smlmtorch.struct_type import struct
...
class SMLMFitter:
    ...
    def process(self, roi_iterator, imgshape, psf_model, fit_constants=None, pb_desc=None):
        ...
        pos = 0
        with tqdm.tqdm(total=numrois) as pb, torch.no_grad():
...
class SFLocalizer:
    def __init__(self, tif_path, *, pattern_frames, pixelsize, 
        ...
        self.cache_dir = dir + f"/cache{cache_tag}/" + os.path.splitext(fn)[0]+ "/"
    ...
    def detect_spots(self, ignore_cache=False, moving_window=False, alt_img_source=None, alt_rois_path=None,
        ...
        cfg = struct(pf=self.pattern_frames.size, 
        ...
        if not cache.is_valid_cache(rois_path, cfg, self.src_path) or ignore_cache:
            ...
            if img_src is None:
                img_src = tiff_read_file(self.src_path, use_tqdm=False, maxframes=self.maxframes)
            ...
            with torch.no_grad():
                numrois, numframes = detection.detect_spots_in_movie(detector, 
                                      ...
                                      totalframes = self.numframes*self.pattern_frames.size if moving_window else self.numframes,
    ...
    def fit_smlm(self, max_crlb_xy=20, max_photons=1e5, min_photons=300, max_chisq=4, ignore_cache=False):
        def process():            
        ...
        if max_crlb_xy is not None:
            ...
            ds = ds[sel]
            ds.save(self.result_dir + f"smlm_crlb{max_crlb_xy:.0f}.hdf5")
        else:
    ...
    def view_rois(self):
        ...
        array_view(combined, title='Spot images and fits')
        
    def fit_simflux(self, mp: pe.ModulationPattern, ds: SFDataset, 
                    iterations=100, lambda_=1e-20, ignore_cache=False, normalizeWeights=False):
    ...
    def estimate_angles(self, pitch_minmax_nm, **kwargs):
        """
    ...
    def estimate_phases(self,mp:pe.ModulationPattern, **kwargs):
