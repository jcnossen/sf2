...
import torch
import numpy as np
import smlmtorch.multipart_tiff as tif
import tqdm
from smlmtorch.gaussian_psf import Gaussian2DFixedSigmaPSF, GaussAstigPSFCalib, Gaussian2DAstigmaticPSF
from fastpsf import Context,CSplineMethods,CSplinePSF,GaussianPSFMethods

import smlmtorch.simflux.pattern_estimator as pe

def blinking_spots(numspots, numframes=2000, avg_on_time = 20, on_fraction=0.1, subframe_blink=4):
    p_off = 1-on_fraction
...
def simulate(tif_fn, mp: pe.ModulationPattern, psf, pos, numframes, em_drift=None, 
             intensity=1000, bg=5, width=100, device=None, **blink_params):
    ...
    with tif.MultipartTiffSaver(tif_fn) as writer,  \
        tqdm.tqdm( enumerate(blinking_spots(len(pos), numframes, **blink_params)), total=numframes) as pb:
