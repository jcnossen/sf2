...
class SpotDetector(torch.nn.Module):
    def __init__(self, detection_threshold, 
        ...
        H,W = image_shape
        bg_filter = ( torch.arange(H)[:,None]-H//2 )**2 + ( torch.arange(W)[None,:]-W//2 )**2 < bg_filter_size**2
    ...
    def detect(self, image):
        ...
        result = conv_psf - conv_bg[None]
        alt_result, z_ = (conv_psf / (conv_bg[None]+1)).max(0)
        ...
        #                     result.cpu().numpy(), alt_result.cpu().numpy()]))
...
def detect_spots_in_movie(detector, camera_calib, movie_iterator, sumframes, totalframes,
                 output_fn, batch_size=10000, callback=None, use_tqdm=True):
    
    dev = detector.device
    
    from contextlib import ExitStack
    
    with open(output_fn, "wb") as f, ExitStack() as s:
        
        if use_tqdm:
            pb = tqdm.tqdm(total = totalframes)
            s.enter_context(pb)
        else:
            pb = None
        ...
        def save_rois():
            ...
            nonlocal batch_info, batch_rois
            
            info = np.concatenate(batch_info)
            pixels = np.concatenate(batch_rois)
            np.save(f, info, allow_pickle=False)
            ...
            #print(f'saved {len(info)}. total rois is now {numrois}. ')
        ...
        for i, img in enumerate(movie_iterator):
            ...
            if nsummed == sumframes:
                ...
                if callback is not None:
                    rois_info, rois = callback(rois_info, rois)
                ...
                #print(f"rois in batch: {rois_in_batch}. len(rois_info): {len(rois_info)}") 
...
if __name__ == '__main__':
    ...
    sd = SpotDetector(10, (W,W), detection_template.to(dev), 20, 
                      bg_filter_size=20, max_filter_size=10)
    ...
    """
    rois_fn = 'test_rois.npy' 
    mov = sample[None].repeat(18,1,1).cpu().numpy()
    nr,nf=detect_spots_in_movie(sd, None, mov, 6, len(mov), rois_fn)
    
    #pixels = load_rois(rois_fn)[1]
    #from ui.array_view import array_view
    #array_view(pixels)
