...
class LocalizationReporter:
    ...
    def draw_patterns(self, dims, **kwargs):
        for ep in tqdm.trange(len(self.mp.mod), desc='Generating modulation pattern plots'):
            self.draw_pattern(ep, dims, **kwargs)

    def draw_pattern(self, ep, dims, me_threshold=0.2, figsize=(8,6), label=None, numpts=2000):
        ds = self.ds
        ...
        normI = I / ds.photons[sel]
        proj = (k[None] * ds.pos[sel][:, :dims]).sum(1) - spot_phases
        x = proj % (np.pi * 2)

        plt.figure(figsize = figsize)
        plt.scatter(x[accepted], normI[accepted], marker='.', c='b', label='Accepted')
        plt.scatter(x[rejected], normI[rejected], marker='.', c='r', label='Rejected')

        sigx = np.linspace(0, 2 * np.pi, 400)
        exc = mod['relint'] * (1 + mod['depth'] * np.sin(sigx))# - mod['phase']))
        plt.plot(sigx, exc, 'k', linewidth=4, label='Estimated P')
        ...
        plt.title(f'{label} Pattern {ep}. K={lenk:.4f} ' + f" Phase={self.mp.mod[ep]['phase'] * 180 / np.pi:.3f}).")
        plt.colorbar()
        plt.legend()
        plt.tight_layout()
